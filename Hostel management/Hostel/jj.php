...
if(isset($_POST['sub2']))
{
	...
	if($res1)
	{
		...
		$sql11="select * from `roomallotment` where `Reg_id`='$a'";
		$res11=mysqli_query($con,$sql11);
		if($res11){
			$sql22="UPDATE `roomallotment` set roomid='$rnid' where `Reg_id`='$a'";
		...
		$sql5="UPDATE `roomallotment` set `status`=0 where `roomid`='$rnid'";
		$res5=msqli_query($con,$sql5);
	}
