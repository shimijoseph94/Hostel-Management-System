<?php  include_once("header3.php");
if(isset($_POST['dealloc']))
{
   $regid = $_POST["rid"];
   echo $regid;
   $query1 = "UPDATE `roomallotment` SET `Status`=1 WHERE `Reg_id`='$regid'";
   echo $query1;
	$result = mysqli_query($con, $query1);
    $query2 = "UPDATE `registration` SET`allocation_status`=0 WHERE `Reg_id`='$regid'";
   echo $query2;
	$result2 = mysqli_query($con, $query2);
	//$row1=mysqli_fetch_array($result1);
	//$rno=$row1['roomid'];
	//echo $rno;
	
	
	//$query3 = "DELETE FROM roomallotment WHERE `Reg_id`='$rno'";
    //echo $query3;
	//$result3 = mysqli_query($con, $query3);

    echo '<script language="javascript">';
    echo 'alert("DEALLOCATED")';
    echo '</script>'; 
	//$query2 = "UPDATE `roominfo` SET `Occupied`= 0 WHERE `roomid`=$rno";
    //echo $query2;
	//$result2 = mysqli_query($con, $query2);
//<script>window.location.assign("secoallo.php")</script>
//<script type="text/javascript">location.href = 'secoallo.php';</script>
//header('Location: secoallo.php');
//header("Location: secoallo.php");

	}
?> 
      <div id="page-wrapper">

        <div class="row">
          <div class="col-lg-12">
            <h1>Student Details</small></h1>
            <ol class="breadcrumb">
              <li><i class="fa fa-dashboard"></i> Student Details</li>
              <li class="active"><i class="fa fa-edit"></i> </li>
            </ol>
            <!--<div class="alert alert-info alert-dismissable">
              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
              Visit <a class="alert-link" target="_blank" href="http://getbootstrap.com/css/#forms">Bootstrap's Form Documentation</a> for more information.
            </div>-->
          </div>
        </div><!-- /.row -->
<body background="images/bg.jpg">
<table border=5px style="width:3px; margin-left:0%; margin-top:10%;color:black;">
<tr><th>Reg_id</th><th>Name</th><th>Age</th><th>Fathersname</th><th>Mothersname</th><th>Emailid</th><th>HouseName</th><th>Place</th><th>Department</th><th>Branch</th><th>Semester</th><th>RoomNo</th><th>ACTION</th></tr>


	
	 <?php
	
 //$sql1=("select r.*,d.Dept_name,s.Sem_name,b.Branch_name  from roomallotment ra join registration r on ra.reg_id=r.Reg_id join department d on r.Dept_id=d.Dept_id
 //join semester s on s.Sem_id=r.Sem_id join Branch B on B.branch_id=r.Branch_id where allocation_status=1 and ra.status=0" );
  $sql1=("select r.*,d.Dept_name,s.Sem_name,b.Branch_name from registration r join department d on r.Dept_id=d.Dept_id
 join semester s on s.Sem_id=r.Sem_id join Branch B on B.branch_id=r.Branch_id where allocation_status=1 " );
	$result1=mysqli_query($con,$sql1);
	while($row=mysqli_fetch_array($result1))
 {
	 $a=$row['Reg_id'];
	 $sqll=mysqli_query($con,"select * from roomallotment where Reg_id='$a'");
	 $row2=mysqli_fetch_array($sqll);
	 
?>


    <tr>	
	<form name="myform" action="#" method="post">
	
	<td><?php echo $row['Reg_id'];?></td>
	<td><?php echo $row['Name'];?></td>
	<td><?php echo $row['Age'];?></td>
	<td><?php echo $row['Fathersname'];?></td>
	<td><?php echo $row['Mothersname'];?></td>
	<td><?php echo $row['Emailid'];?></td>
	<td><?php echo $row['HouseName'];?></td>
	<td><?php echo $row['Place'];?></td>
 <td><?php echo $row['Dept_name'];?></td>
	<td><?php echo $row['Branch_name'];?></td>
	<!--<td><input type="text" name="Semester" value="--><td><?php echo $row['Sem_name'];?></td>
	
	
	<td><?php echo $row2['roomid'];?></td>
	
	 <input type="hidden" name="rid" value="<?php echo $row['Reg_id'];?>"></input>
	 <td><input type="submit" value="DEALLOCATE" name="dealloc"></td>
	 

	 </form>
	
	 
	 
	 
	</tr>
	</form>
	
	
	
	
    <?php
 }
 ?>
 
   </div>
        </div><!-- /.row -->

      </div><!-- /#page-wrapper -->

    </div><!-- /#wrapper -->

    <!-- JavaScript -->
    <script src="js/jquery-1.10.2.js"></script>
    <script src="js/bootstrap.js"></script>

  </body>
</html>