...
a:
$qry="SELECT * FROM `registration` WHERE `Dept_id`='$depid' and `Sem_id`='$semid' and `Branch_id`='$brid' and `allocation_status`=0";
$r2= mysqli_query($con,$qry);
while($row=mysqli_fetch_array($r2))
{		
	$abc="SELECT * FROM  `roominfo` WHERE  `capacity` >`Occupied` and `capacity`=$rcap";
	...
	while($row1=mysqli_fetch_array($r1)){
		...
		for($i=$rcapacity;$i>0;$i--)
		{
			if($occ<$cap)
			{
				...
				$r5=mysqli_query($con,"UPDATE `roominfo` SET `Occupied`=`Occupied`+1 WHERE `roomid`='$rno'");
				goto a;
			}
		}
	}
...
<td><input type="submit" name="sub2" value="allocate" Onclick = "alert('are you want to confirm')" ></td>
